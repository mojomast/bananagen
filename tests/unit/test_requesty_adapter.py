...
@pytest.fixture
def adapter():
    """Create a RequestyAdapter instance."""
...
class TestRequestyAdapter:
    """Test RequestyAdapter class."""

    @patch('bananagen.adapters.requesty_adapter.decrypt_key')
    ...
    async def test_call_gemini_success_with_image_url(self, mock_session_class, mock_decrypt_key, adapter, temp_image):
        ...
        with patch('builtins.open', mock_open()), \
             ...
             patch('bananagen.adapters.requesty_adapter.hashlib.sha256') as mock_sha:
            ...
            assert metadata['requesty_response_id'] == "req-123"
    ...
    async def test_call_gemini_success_with_base64_content(self, mock_session_class, mock_decrypt_key, adapter, temp_image):
        ...
        with patch('builtins.open', mock_open()), \
             patch('os.path.exists', return_value=True), \
             patch('bananagen.adapters.requesty_adapter.hashlib.sha256') as mock_sha, \
             patch('bananagen.adapters.requesty_adapter.base64.b64decode') as mock_b64decode:
            ...
            assert metadata['requesty_response_id'] == "req-456"
    ...
    async def test_call_gemini_fallback_to_placeholder(self, mock_session_class, mock_decrypt_key, adapter, temp_image):
        ...
        with patch('builtins.open', mock_open()), \
             patch('os.path.exists', return_value=True), \
             patch('bananagen.adapters.requesty_adapter.hashlib.sha256') as mock_sha, \
             patch('bananagen.adapters.requesty_adapter.Image') as mock_image_class:
            ...
            assert metadata['requesty_response_id'] == "req-fallback"
